...
class HMIVDCMA:
    ...
    def __init__(self, func, xmean0, sigma0, domain_int, margin, lamb, budget, sd, **kwargs):
        np.random.seed(sd)
        ...
        self.all_v = []
        self.plot_iter_f = []
        self.plot_iter_mean = {f"mean_{i}th": [] for i in range(self.N)}
        self.iters = 0
        ...
        self.maxeval = kwargs.get('maxeval', budget)
    ...
    def run(self):
        ...
        while not satisfied:
            ...
            #if satisfied:
            #    print(condition)
        return self.is_success, self.x_best, self.fbest, self.neval, self.all_inds, self.all_mean, self.all_std, self.all_v, self.plot_iter_f, self.plot_iter_mean
    ...
    def _onestep(self):
        ...
        #print('Mean:', self.xmean)
        tmp_std = self.sigma * np.sqrt(self.dig)
        m_cp = self.xmean.copy()
        #print('inds:', arx)
        #print('1: ', arx[:, 0])
        #print('2:', arx[:, 1])
        self.all_inds.append([arx[:,0], arx[:, 1]])
        #print('test: ', [arx[:,0], arx[:, 1]])
        self.all_mean.append(np.array(m_cp.flatten()))
        self.all_v.append(np.array(self.vvec.flatten()))
        self.all_std.append(np.array(tmp_std.flatten()))

        #print('first:', arx[0, 1], arx[0, 2])
        l_close = np.array([
        ...
        self.plot_iter_f.append([f_best, self.iters])
        for kk in range(self.N):
            id = f"mean_{kk}th"
            self.plot_iter_mean[id].append([float(self.xmean[kk]), self.iters])
        ...
        condition_bias = (resolution <= 1) & ~(((self.dx)[self.dim_co:] < 0.0) ^ (self.xmean[self.dim_co:] - l_close < 0.0))#.reshape(self.dim_int)
        #print('cond:', condition_bias)
        ...
        if self.ssa == 'MCSA':
            ...
            if self.flg_injection:
                ...
                #print('idx:', idx)
                #print('idx==1:', np.where(idx == 1)[0][0])
                #print('idx==0:', np.where(idx == 0)[0][0])
                alpha_act /= float(self.lam - 1)
                #print('alpha_act:', alpha_act)
        ...
        self.xmean[self.dim_co:] = np.array([
            ...
            for i in range(self.dim_int)])
        #print('Step-size coordinate:', self.sigma * np.sqrt(Cnew[self.dim_co:]))
        #print('Mean:', self.xmean)
    ...
    def _check(self):
        ...
        if self.fbest < self.ftarget:
            #print(f"xbest: {self.x_best}, fbest: {self.fbest}")
